...
import com.school.config.KafkaProperties;
...
class SchoolKafkaResourceIT {
    ...
    void setup() {
        ...
        SchoolKafkaResource kafkaResource = new SchoolKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/school-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/school-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
